...
class SystemManager(object):
    ...
    def start_processor(self):
        ...
        self.processor = Processor(
            systemManager=self, queue=self.processor_task_queue)
    ...
    def start_long_scheduler(self):
        self.longScheduler = LongTermScheduler(
            systemManager=self, processList=self.system_process_list)
    ...
    def start_io_device(self, device):
        self.devices[device] = IODevice(
            device, systemManager=self, processList=self.system_process_list)
        self.devices[device].setDaemon(True)
        self.devices[device].start()
        Logger().log(["{0} Device Started".format(device)])

    def start_all_system_device(self):
        Logger().log(["Init all system devices "])
    ...
    def stop_all_system_device(self):
